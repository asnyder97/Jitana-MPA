	...
	for (unsigned i = 1; i < methodVector.size(); ++i)
	{
		...
		while(!insn.eof())
		{
			...
			if (first < 100)
			{
				...
				for (unsigned k = insnVector.size() - count; k < insnVector.size(); ++k)
				{
					if (output[1] == insnVector[k][1])
					{
						alreadyIn = true;
					}
				}
...
void parseIACIntent(char *path)
{
	...
	while(!IAC.eof())
	{
		...
		if (connection < 100)
		{
			...
			if (oldReceiver == receiver && oldSender == sender)
			{
			...
			if (sender != receiver && !alreadyConnected)
			{
...
void printIacVector()
{
	for (unsigned n = 0; n < iacVector.size(); ++n)
	{
		cout << "SENDER = " << iacVector[n][0] << "\n";
		cout << "INTENT = " << iacVector[n][1] << "\n";
		cout << "RECEIVER = " << iacVector[n][2] << "\n\n";
	}
}
...
void printINSN()
{
	for (unsigned n = 0; n < insnVector.size(); ++n)
	{
		cout << "URL = " << insnVector[n][0] << "\n";
		cout << "STRING = " << insnVector[n][1] << "\n";
		cout << "STRING = " << insnVector[n][2] << "\n\n";
	}
}
...
void printMethod()
{
	for (unsigned u = 0; u < methodVector.size(); ++u)
	{
		cout << "METHOD = " << methodVector[u][0] << "\n";
		cout << "ADDITONAL INFO = " << methodVector[u][1] << "\n";
		cout << "URL = " << methodVector[u][2] << "\n\n";
	}
}
...
int main(int argc, char *argv[])
{
	...
	parseMethods( argv[2] );
	parseInsn( argv[2] );
	cout << methodVector.size() << "\n";
	cout << insnVector.size() << "\n";
	...
	findSensitivePath(connections, methodVector, blackListID,  argv[3], argv[4], argv[2]);
	final = isSensitive(final);
